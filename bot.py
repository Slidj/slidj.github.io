...
NEWS_TEMPLATE = None
# MARKDOWN_CHARS ВИДАЛЕНО

# ----------------------------------------------------
# 2. ФУНКЦІЇ ФОРМАТУВАННЯ ТА ЗАВАНТАЖЕННЯ ШАБЛОНУ
# ----------------------------------------------------

# Функція escape_markdown ВИДАЛЕНА
...
def _process_article(article, title_prefix, source_key, url_key, image_key=None, description_key='description'):
    ...
    # Очищуємо від HTML (Цей крок залишаємо для максимальної чистоти вхідного контенту)
    ...
    # ЕКРАНУВАННЯ ВИДАЛЕНО - ВІДТЕПЕР ТІЛЬКИ ЧИСТИЙ ТЕКСТ
    safe_title = title
    safe_source = source
    safe_url = url
