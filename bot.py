...
# Цей шлях вказує на файл, який ви завантажили у свій репозиторій.
...
def _get_image_with_title(image_url: str, title: str) -> bytes | None:
    ...
    3. Накладає жирний заголовок (з використанням FONT_PATH).
    ...
    # *** ВИПРАВЛЕННЯ ШРИФТУ: Встановлюємо більший розмір (42pt) ***
    font_size_target = 42 
    
    try:
        # 1. Спроба завантажити локально наданий шрифт з папки fonts/
        font = ImageFont.truetype(FONT_PATH, font_size_target)
        print(f"Using custom font from {FONT_PATH} with size {font_size_target}.")
    except Exception:
        # 2. Якщо файл не знайдено, використовуємо шрифт за замовчуванням (який буде мілким)
        print(f"Custom font not found at {FONT_PATH}. Using small default font.")
        font = ImageFont.load_default()
        
    # *** ВИПРАВЛЕННЯ: Обмеження ширини для перенесення слів, щоб не виходило за рамки ***
    wrap_width = 35 if fallback_used else 45 
    title_wrapped = textwrap.fill(title, width=wrap_width) 
    
    # ВИПРАВЛЕННЯ PIL: ВИКОРИСТАННЯ textbbox для розрахунку розміру
    try:
        left, top, right, bottom = draw.textbbox((0, 0), title_wrapped, font=font)
        text_width = right - left
        text_height = bottom - top
    except Exception as e:
        print(f"Error calculating text size with textbbox: {e}. Estimating size.")
        # Запасний розрахунок (якщо textbbox недоступний)
        text_width = 600
