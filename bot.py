...
def escape_html(text):
    """
    Критичне виправлення: Екранує символи <, > та & для безпечного використання 
...
def _fetch_news(params: dict, title_prefix: str) -> dict | None:
    ...
    try:
        ...
        if data['status'] == 'ok' and data['articles']:
            ...
            # ❗ КРИТИЧНЕ ВИПРАВЛЕННЯ: ЕКРАНУЄМО СПЕЦІАЛЬНІ СИМВОЛИ ДЛЯ БЕЗПЕКИ HTML
...
def _send_text_message(text_content):
    """Надсилає повідомлення як HTML ( sendMessage )."""
...
def publish_endpoint():
    ...
    if image_url:
        ...
        # Перетворюємо '\n' на HTML-переноси <br> для sendPhoto caption
    ...
    if _send_text_message(caption): # Відправляємо оригінальний caption (з \n)
