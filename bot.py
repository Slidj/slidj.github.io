...
MIN_IMAGE_SIZE = 600 # Збільшимо мінімальний розмір для кращої підложки
# ПРИМІТКА: Потрібно завантажити обидва файли у папку 'fonts'
...
def _load_template(file_path="templates/news_template.txt"):
    # Цей шаблон тепер потрібен лише для отримання даних, але ми його не використовуємо для фінального тексту
    ...
    try:
        ...
        # Якщо шаблону немає, використовуємо внутрішній шаблон
...
def _generate_news_image(image_url: str, title: str, description: str, source: str) -> bytes | None:
    """
    Генерує фінальне зображення, на яке накладається весь текст новини.
    """
    
    # 1. СТВОРЕННЯ/ОТРИМАННЯ ФОНУ
    final_image = None
    width, height = 1000, 600 # Стандартний розмір підложки
    ...
    try:
        if image_url:
            ...
            with Image.open(io.BytesIO(img_data)) as img:
                img = img.convert("RGB")
                # Масштабування зображення до 1000x600, обрізаючи його
                img_ratio = img.width / img.height
                target_ratio = width / height
                
                if img_ratio > target_ratio:
                    # Зображення ширше, обрізаємо по горизонталі
                    new_width = int(img.height * target_ratio)
                    left = (img.width - new_width) // 2
                    img = img.crop((left, 0, left + new_width, img.height))
                else:
                    # Зображення вище, обрізаємо по вертикалі
    ...
    try:
        ...
        font_body = ImageFont.truetype(FONT_REGULAR_PATH, 24)
        font_source = ImageFont.truetype(FONT_REGULAR_PATH, 18)
    except Exception as e:
    ...
    current_y += (bottom - top) + 30 # Перехід на наступну секцію


    # B. ОПИС ТА ТІЛО НОВИНИ
    description_wrapped = textwrap.fill(description, width=90)
    
    draw.multiline_text((padding, current_y), description_wrapped, font=font_body, fill='white')

    # Оцінка висоти опису для визначення місця джерела
    # NOTE: тут немає точного способу без циклу, робимо грубу оцінку
    try:
         _, _, _, desc_bottom = draw.textbbox((padding, current_y), description_wrapped, font=font_body)
         current_y = desc_bottom + 30
    except:
         # Груба оцінка, якщо немає textbbox
...
def _process_article(article, title_prefix, source_key, url_key, image_key=None, description_key='description'):
    ...
    # Повертаємо всі необхідні дані для генерації зображення
    return {
        'title': safe_title, 
        'image_url': image_url, 
        'url': url, 
...
def _send_text_message(text_content, url):
    ...
    # У цьому режимі, якщо sendPhoto не вдасться, ми відправляємо тільки URL
...
def publish_endpoint():
    ...
    # Отримуємо всі дані для вбудовування в зображення
    ...
    if processed_image_bytes:
        ...
        # Caption залишаємо порожнім або коротким, оскільки вся новина вже на зображенні
        payload = {
            'chat_id': CHANNEL_ID,
