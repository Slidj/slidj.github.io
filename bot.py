...
import datetime 
import io
import textwrap # Додано для обгортання тексту
from collections import deque
from flask import Flask
from PIL import Image, ImageDraw, ImageFont # Додано для роботи з зображеннями
...
MIN_IMAGE_SIZE = 400 # Мінімальний розмір зображення (обидві сторони)
# Шлях до шрифту для накладання тексту. На Render-серверах часто працює цей шлях:
FONT_PATH = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf" 
# Якщо цей шлях не працює, PIL автоматично перейде на ImageFont.load_default().
...
def _get_image_with_title(image_url: str, title: str) -> bytes | None:
    ...
    # Намагаємося завантажити жирний шрифт
    font_size = 36 
    try:
    ...
    # Розумне перенесення тексту
    # Обмежуємо ширину, якщо використовуємо заглушку (менше тексту на заглушці)
    wrap_width = 40 if fallback_used else 60
    title_wrapped = textwrap.fill(title, width=wrap_width) 
    
    # Розрахунок позиції (центр знизу)
    # NOTE: draw.textsize застаріла, але працює. У нових версіях PIL використовують draw.textlength
    # Для сумісності залишаємо draw.textsize
    text_width, text_height = draw.textsize(title_wrapped, font=font)
    x = (final_image.width - text_width) / 2
    y = final_image.height - text_height - 20 
    
    # Додавання тексту (з контуром для кращої читабельності)
    text_color = 'white'
    outline_color = 'black'
    ...
    byte_io = io.BytesIO()
    # Telegram краще працює з JPEG для sendPhoto
...
def publish_endpoint():
    ...
    article_title = news_data.get('title') # Отримуємо заголовок
    ...
    # Отримуємо зображення у форматі bytes, яке вже має накладений заголовок
    ...
    if processed_image_bytes:
        ...
        # Надіємося, що Telegram сам розбере рядок JSON в reply_markup
        payload = {
            'chat_id': CHANNEL_ID,
            'caption': caption,
            'reply_markup': keyboard 
        ...
        try:
            # Важливо: використовуємо data=payload для параметрів, і files=files для самого зображення
