...
# ----------------------------------------------------
# 2. ФУНКЦІЯ ОТРИМАННЯ НОВИНИ (ВИПРАВЛЕНО ДЛЯ УКРАЇНСЬКОЇ МОВИ)
# ----------------------------------------------------

def get_latest_news() -> str:
    """Отримує одну світову новину, фільтруючи її за українською мовою ('uk')."""
    params = {
        'apiKey': NEWS_API_KEY,
        # Шукаємо топові світові заголовки (без обмеження країною), але лише українською мовою
        'language': 'uk', 
        'category': 'general',
        'pageSize': 1
    }
    ...
    try:
        ...
        if data['status'] == 'ok' and data['articles']:
            ...
            print(f"Помилка NewsAPI (не знайдено статей українською мовою) або пуста відповідь: {data.get('code')}")
            return None # Повертаємо None, якщо новин немає
            
    except requests.exceptions.RequestException as e:
        print(f"Помилка з'єднання з NewsAPI (можливо, недійсний ключ API): {e}")
        return None
