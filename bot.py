...
import random 
import datetime # <--- ДОДАНО: Для отримання поточної дати та часу
...
# ----------------------------------------------------
# 2. ФУНКЦІЇ ОТРИМАННЯ НОВИНИ (З РЕЗЕРВУВАННЯМ ТА ВИПАДКОВИМ ВИБОРОМ)
# ----------------------------------------------------
...
def _fetch_news(params: dict, title_prefix: str) -> str | None:
    ...
    try:
        ...
        if data['status'] == 'ok' and data['articles']:
            ...
            # ВИБИРАЄМО ВИПАДКОВУ СТАТТЮ З ОТРИМАНИХ 5
            ...
            # Отримуємо час публікації
            # Формат: День.Місяць.Рік Час:Хвилини
            current_time = datetime.datetime.now().strftime("%d.%m.%Y %H:%M")
            
            # ПОКРАЩЕНЕ ФОРМАТУВАННЯ З MARKDOWN ТА ЕМОДЗІ (Варіанти 3 та 4)
            return (
                f"📢 **{title_prefix}** | _{current_time}_\n" # Додаємо час
                f"========\n" # Подвійний роздільник
                f"**{title}**\n\n"
                f"_🔎 Джерело: {source}_" # Курсив та емодзі
