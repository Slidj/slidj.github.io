...
UNICODE_BOLD_MAP = {
    ...
    # Кириличні (проста заміна, де доступно)
...
def _process_article(article, title_prefix, source_key, url_key, image_key=None, description_key='description'):
    ...
    # Форматуємо лише мітки, залишаючи вміст чистим
    ...
    # Створюємо нові змінні для шаблону, які включають жирну мітку та дані
    ...
    # Час залишається чистим (для уникнення проблем з цифрами/двокрапками)
    ...
    caption = caption.replace('{{title_line}}', styled_title_line) # НОВА ЗМІННА
    caption = caption.replace('{{source_line}}', styled_source_line) # НОВА ЗМІННА
    caption = caption.replace('{{url_line}}', styled_url_line) # НОВА ЗМІННА
