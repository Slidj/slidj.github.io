...
import random 
import datetime 
import re # Додаємо для видалення HTML
...
def load_template():
    ...
    try:
        ...
        print(f"Error loading template (using fallback): {e}")
        # Використовуємо максимально простий резервний шаблон
        NEWS_TEMPLATE = (
...
def escape_html(text):
    """Видаляємо &gt;, &lt; та &amp; (екранування більше не потрібне, оскільки ми не використовуємо HTML)"""
    if text is None:
        return ""
...
def _fetch_news(params: dict, title_prefix: str) -> dict | None:
    ...
    try:
        ...
        if data['status'] == 'ok' and data['articles']:
            ...
            # Ми не екрануємо &<> тут, тому що потім видалимо всі HTML-теги
...
def publish_endpoint():
    ...
    # 🌟 ВИПРАВЛЕННЯ: ЗАПОБІГАННЯ ПОМИЛЦІ 500, якщо новин немає
    if not news_data:
    ...
    if caption:
        ...
        clean_text = "Новина без заголовка (помилка шаблону)."
    ...
    payload = {
        'chat_id': CHANNEL_ID,
        'text': clean_text, 
        # 'parse_mode' не використовуємо, тому що текст чистий
    }
    ...
    try:
        ...
        # 🌟 ВИПРАВЛЕННЯ: Повертаємо 200 OK, щоб UptimeRobot не скаржився на 500
