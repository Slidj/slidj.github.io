...
# === НОВЕ: Змінна для відстеження ротації ===
...
# Шаблон для привітання (залишаємо без змін)
GREETING_TEMPLATE = (
...
# (Функції _load_template, _clean_text та _get_image_with_title залишаються без змін)
...
def _get_image_with_title(image_url: str, title: str) -> bytes | None:
    """
    1. Обрізає заголовок, якщо він занадто довгий.
    2. Завантажує та перевіряє зображення, або генерує заглушку.
...
# ----------------------------------------------------
# 3. ДОПОМІЖНІ ФУНКЦІЇ ДЛЯ API (Змінено _fetch_newsdata, додано _fetch_global_newsdata)
# ----------------------------------------------------
...
# === ЗМІНА: ФУНКЦІЯ ДЛЯ УКРАЇНСЬКИХ НОВИН (NewsData.io) ===
...
# === НОВЕ: ФУНКЦІЯ ДЛЯ СВІТОВИХ НОВИН УКРАЇНСЬКОЮ (NewsData.io) ===
def _fetch_global_newsdata(title_prefix: str) -> dict | None:
    ...
    params = {
        ...
        # 'country' відсутній, щоб брати світові
        'size': 5
    }
    ...
    try:
        ...
        if data['status'] == 'success' and data['results']:
            ...
            for article in articles:
                # Додаємо фільтр, щоб уникнути українських новин у світовій стрічці
                source_country = article.get('country', 'XX').lower()
...
# === ЗМІНА: ФУНКЦІЯ РОТАЦІЇ ТА ФОЛБЕКУ ===
def get_latest_news() -> dict | None:
    global LAST_FETCH_WAS_UKRAINE
    
    # 1. Визначаємо, що шукати зараз (чергування)
    if LAST_FETCH_WAS_UKRAINE:
        ...
        # Якщо світові не знайшли, спробуємо знайти українські
        print("Global News not found. Falling back to Ukraine News...")
        news_data = _fetch_ukraine_newsdata("Свіжа Новина з України")
        # Тут не міняємо LAST_FETCH_WAS_UKRAINE, оскільки наступний цикл має початися з України
        ...
        # Якщо українські не знайшли, спробуємо знайти світові
        print("Ukraine News not found. Falling back to Global News...")
        news_data = _fetch_global_newsdata("Світова Новина")
        # Тут не міняємо LAST_FETCH_WAS_UKRAINE, оскільки наступний цикл має початися з Global
        
    if news_data:
        return news_data
        
    # 2. Фолбек на NewsAPI, якщо NewsData.io повністю відмовив
...
# (Функції _create_inline_keyboard, _send_text_message, _send_and_pin_greeting 
# та ендпоінти /publish, /greet, /home залишаються без змін)
