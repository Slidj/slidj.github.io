    ...
    try:
        ...
        if data['status'] == 'ok' and data['articles']:
            ...
            for art in articles:
                ...
                # Додаємо перевірку на наявність зображення, щоб збільшити шанси на успіх sendPhoto
            ...
            url = article.get("url", "#") 
            image_url = article.get("urlToImage") # Знову отримуємо image_url
...
# Ця функція допомагатиме у разі помилки sendPhoto
def _send_text_message(text_content):
    """Надсилає повідомлення як чистий текст ( sendMessage ) з parse_mode=HTML."""
    telegram_publish_url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    payload = {
        'chat_id': CHANNEL_ID,
        'text': text_content, 
        'parse_mode': 'HTML' # Знову використовуємо HTML, оскільки ми перевірили, що текст чистий
    }
    
    try:
        response = requests.post(telegram_publish_url, data=payload)
        response.raise_for_status()
        print("Fallback to sendMessage successful.")
        return True
    except requests.exceptions.RequestException as e:
...
def publish_endpoint():
    ...
    # --- ЕКРАНУВАННЯ ДЛЯ БЕЗПЕКИ HTML В TELEGRAM ---
    # Перетворюємо переноси рядка (\n) назад на <br> для коректного відображення HTML в Telegram
    html_caption = caption.replace('\n', '<br>')
    
    # -------------------------------------------------------------------------
    # 1. СПРОБА #1: SENDPHOTO (з URL зображення)
    # -------------------------------------------------------------------------
    ...
    if image_url:
        ...
        try:
            ...
            print("Публікація sendPhoto успішна.")
            return "News published successfully (with photo)!", 200
            
        except requests.exceptions.RequestException as e:
            # Якщо sendPhoto не спрацював (наприклад, 400 Bad Request, зображення недійсне)
    ...
    # -------------------------------------------------------------------------
    # 2. СПРОБА #2: FALLBACK НА SENDMESSAGE (Тільки текст)
    # -------------------------------------------------------------------------
    
    if _send_text_message(html_caption):
        return "News published successfully (fallback to text only)!", 200
    else:
        # Повертаємо 200 OK, щоб UptimeRobot не скаржився
        return "Telegram final send failed.", 200 

@app.route('/', methods=['GET'])
def home():
    return "Bot is alive and ready to publish.", 200

# !!! Виклик load_template більше не потрібен !!!
