        ...
        try {
            ...
            if (data.images?.logos?.length > 0) {
                ...
                if (logo && title) {
                    ...
                    title.innerHTML = `<img src="${logoUrl}" alt="${movie.title}" class="nf-logo" style="max-height: 120px;">`;
                }
            }
        } catch (e) { }
    }
...
export async function openMoviePage(movie) {
    ...
    window.ui_openTrailer = (key) => {
        ...
        f.src = `https://www.youtube.com/embed/${key}?autoplay=1`;
        p.style.display = 'flex';
    };

    // 🔥 ФУНКЦІЯ SHARE (ПОВЕРНУТО)
    window.ui_share = (id) => { 
        let m = state.activeMovie || state.feedMovies.find(i=>i.id==id); 
        if(!m) return; 
        // Формуємо пряме посилання для Telegram Mini App
