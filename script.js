...
const TMDB_KEY = "4dac8d33b5f9ef7b7c69d94b3f9cd56b"; 
...
if (window.Telegram && window.Telegram.WebApp) {
    ...
    if (user) {
        document.getElementById('header_title').innerText = user.first_name + (user.last_name ? ` ${user.last_name}` : '');
        if (user.photo_url) {
...
async function fetchContent(isLoadMore = false) {
    ...
    try {
        if (appMode === 'news') await fetchNews(isLoadMore);
        else if (appMode === 'movies') await fetchMovies(isLoadMore);
    } catch (error) {
        console.error(error);
...
function showState(type, errorDetails = "") {
    const container = document.getElementById('content_container');
    container.innerHTML = `<div class="empty-state"><div class="empty-text">Статус: ${type}</div><div class="empty-subtext">${errorDetails}</div></div>`;
...
window.performSearch = function() {
    ...
    feedNews = []; feedMovies = []; newsPageToken = null; moviePage = 1;
    fetchContent();
};

window.loadMore = function() { fetchContent(true); };
